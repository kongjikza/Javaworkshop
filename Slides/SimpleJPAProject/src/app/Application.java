...
public class Application {
	...
	public static void main(String[] args) {
		...
		//sample.jpa ชื่อต้องตรงกับ persistence
		EntityManager em = Persistence.createEntityManagerFactory("sample.jpa").createEntityManager();

		// Begin transaction
		em.getTransaction().begin();

		ProductCategory oldProductCategory = em.find(ProductCategory.class, 1); // คือการ query 1  ตือ primarykey
		...
		newProduct1.setProductCategory(newProductCat); // set ให้ Object มีความสัมพันธ์กัน
		newProductCat.getProducts().add(newProduct1); // set ให้มันรู้จักกัน
		...
		for (Product p : em.find(ProductCategory.class, 1).getProducts()) { // Query จาก DB
